...
export function createRecord(userFilename, extension, mimeType) {
  ...
  let storageBasename = userFileUuid + '.' + extension;
  let storagePath = Path.join(Config.STORAGE_PATH, storageBasename);
  let storageFileUri = global.encodeURI(`share://` + storageBasename);
